...
def _counter_templates(
    ...
    if reject and len(standard_states) > 0:
        standard_states.append(STATE_OFF)
...
def _super_counter_templates(
    ...
    area_regex = f"_area_{area.id}" if area is not None else "_area_(?:(?!_type_).)*"
    regex = f"binary_sensor\\.{DOMAIN}{area_regex}_type_{prefix}(?:{'|'.join(entity_types)})"
...
async def _create_counter(
    ...
    device_id = f"{DOMAIN}_{area_string}type_{name}"
